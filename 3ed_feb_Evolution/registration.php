...
<body>
    ...
    <form action="registration.php" method="POST">
        <div class="user-details">
            <h3>REGISTRATION</h3>
            <?php if (isset($_GET['prof'])) { echo '<a href="blogPost.php">BlogPost</a><br><br>';}?>
            ...
            <div name="user-firstName">
                ...
                <input type="text" name="<?php echo $firstName; ?>" value="<?php echo getValues("user","firstName"); ?>" placeholder="First Name">
                <span class="error"><?php echo isset($firstNameErr) ? $firstNameErr : ""; ?></span>
            </div>
            <div name="user-lastName">
                <?php $lastName = "user[lastName]"?>    
                <label>Last Name:-</label>
                <input type="text" name="<?php echo $lastName; ?>" placeholder="Last Name" value="<?php echo getValues("user","lastName"); ?>">
                <span class="error"><?php echo isset($lastNameErr) ? $lastNameErr : ""; ?></span>
            </div>
            <div name="user-email">
                <?php $email = "user[email]"?>    
                <label>Email:-</label>
                <input type="email" name="<?php echo $email; ?>" value="<?php echo getValues("user","email"); ?>">
                <span class="error"><?php echo isset($emailErr) ? $emailErr : ""; ?></span>
            </div>
            <div name="user-phone">
                <?php $phone = "user[phoneNumber]"?>    
                <label>Mobile number:-</label>
                <input type="number" name="<?php echo $phone; ?>" value="<?php echo getValues("user","phoneNumber"); ?>">
                <span class="error"><?php echo isset($phoneErr) ? $phoneErr : ""; ?></span>
            </div>
            <div name="user-password">
                <?php $password = "user[password]"?>    
                <label>PassWord:-</label>
                <input type="password" name="<?php echo $password; ?>" value="<?php echo getValues("user","password"); ?>">
            </div>
            <div name="user-confirmPassword">
                <?php $confirmPassword = "user[confirmPassword]"?>    
                <label>ConfirmPassWord:-</label>
                <input type="password" name="<?php echo $confirmPassword; ?>" value="<?php echo getValues("user","password"); ?>">
                <span class="error"><?php echo isset($confirmPassWordErr) ? $confirmPassWordErr : ""; ?></span>
            </div>
            <div name="user-information">
                <label>INFORMATION</label>
                <input type="text" name="user[information]" value="<?php echo getValues("user","information"); ?>" style="height: 30px;">
            </div>
        </div>
        <?php if (isset($_GET['prof'])) { die();}?>
